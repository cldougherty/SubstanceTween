...
{ // This script automatically gets added to the prefab once it is created
                            ...
                            if (j < totalListProperties)
                            {
                                ...
                                if (propType == ProceduralPropertyType.Float)
                                {
                                    ...
                                    /*for (int k = 0; k < MaterialVariableKeyframeList[i].myValues.Count; k++)
                                    {
                                        ...
                                        {
                                            float curFloat = float.Parse(MaterialVariableKeyframeList[i].myValues[k]);
                                            MaterialVariableKeyframeDictionaryList[i].PropertyDictionary.Add(materialVariable.name, curFloat);
                                            MaterialVariableKeyframeDictionaryList[i].PropertyFloatDictionary.Add(materialVariable.name, curFloat);
                                        }
                                    }*/
    ...
    void Start()
    {
        ...
        {// if you have 1 keyframe set the substance parameters from that keyframe if you have more than 1 it will animate in Update()
            ...
            if (useSharedMaterial && rend && rend.sharedMaterial.HasProperty("_EmissionColor"))
                rend.sharedMaterial.SetColor("_EmissionColor", MaterialVariableKeyframeDictionaryList[0].emissionColor);
            else if (!useSharedMaterial && rend && rend.sharedMaterial.HasProperty("_EmissionColor"))
                    ...
                    for (int j = 0; j <= keyFrameTimes.Count - 1; j++)
                    {
                        foreach (KeyValuePair<string, Vector4> keyValue in MaterialVariableKeyframeDictionaryList[j].PropertyVector4Dictionary)
                        {
                            if (keyValue.Key == materialVariables[i].name)
                            {
                                ...
                                else if (j > 0 && cacheAtStartup && keyValue.Value != propertyVector4AnimationCheck)
                                {
                                    substance.CacheProceduralProperty(keyValue.Key, true);
                                    animatedMaterialVariables.Add(MaterialVariable);
                                }
                                else if (j == keyFrameTimes.Count - 1 && keyValue.Value == propertyVector4AnimationCheck)
                                {
                                    for (int k = 0; k <= keyFrameTimes.Count; k++)
                    ...
                    for (int j = 0; j <= keyFrameTimes.Count - 1; j++)
                    {
                        foreach (KeyValuePair<string, Vector3> keyValue in MaterialVariableKeyframeDictionaryList[j].PropertyVector3Dictionary)
                        {
                            if (keyValue.Key == materialVariables[i].name)
                            {
                                ...
                                else if (j > 0 && cacheAtStartup && keyValue.Value != propertyVector3AnimationCheck)
                                {
                                    substance.CacheProceduralProperty(keyValue.Key, true);
                                    animatedMaterialVariables.Add(MaterialVariable);
                                }
                                else if (j == keyFrameTimes.Count - 1 && keyValue.Value == propertyVector3AnimationCheck)
                                {
                                    for (int k = 0; k <= keyFrameTimes.Count; k++)
                    ...
                    for (int j = 0; j <= keyFrameTimes.Count - 1; j++)
                    {
                        foreach (KeyValuePair<string, Vector2> keyValue in MaterialVariableKeyframeDictionaryList[j].PropertyVector2Dictionary)
                        {
                            if (keyValue.Key == materialVariables[i].name)
                            {
                                ...
                                else if (j > 0 && cacheAtStartup && keyValue.Value != propertyVector2AnimationCheck && (keyValue.Key[0] != '$' || (keyValue.Key[0] == '$' && animateOutputParameters)))
                                {
                                    substance.CacheProceduralProperty(keyValue.Key, true);
                                    animatedMaterialVariables.Add(MaterialVariable);
                                }
                                else if (j == keyFrameTimes.Count - 1 && keyValue.Value == propertyVector2AnimationCheck || (keyValue.Key[0] == '$' && !animateOutputParameters))
                                {
                                    for (int k = 0; k <= keyFrameTimes.Count; k++)
                    ...
                    for (int j = 0; j <= keyFrameTimes.Count - 1; j++)
                    {
                        foreach (KeyValuePair<string, int> keyValue in MaterialVariableKeyframeDictionaryList[j].PropertyEnumDictionary)
                        {
                            if (keyValue.Key == materialVariables[i].name)
                            {
                                ...
                                else if (j > 0 && cacheAtStartup && keyValue.Value != propertyEnumAnimationCheck)
                                {
                                    substance.CacheProceduralProperty(keyValue.Key, true);
                                    animatedMaterialVariables.Add(MaterialVariable);
                                }
                                else if (j == keyFrameTimes.Count - 1 && keyValue.Value == propertyEnumAnimationCheck)
                                {
                                    for (int k = 0; k <= keyFrameTimes.Count; k++)
                                    {
                    ...
                    for (int j = 0; j <= keyFrameTimes.Count - 1; j++)
                    {
                        foreach (KeyValuePair<string, bool> keyValue in MaterialVariableKeyframeDictionaryList[j].PropertyBoolDictionary)
                        {
                            if (keyValue.Key == materialVariables[i].name)
                            {
                                ...
                                else if (j > 0 && cacheAtStartup && keyValue.Value != propertyBoolAnimationCheck)
                                {
                                    substance.CacheProceduralProperty(keyValue.Key, true);
                                    animatedMaterialVariables.Add(MaterialVariable);
                                }
                                else if (j == keyFrameTimes.Count - 1 && keyValue.Value == propertyBoolAnimationCheck)
                                {
                                    for (int k = 0; k <= keyFrameTimes.Count; k++)
                                    {
    ...
    void Update()
    {
        if (animationToggle)
        {
            ...
            if (!rebuildSubstanceImmediately && (animateObjectBasedOnPosition == AnimateObjectBasedOnPosition.None || (animateObjectBasedOnPosition != AnimateObjectBasedOnPosition.None && playerDistanceToObject <= 10 )) )
                substance.RebuildTextures();
            else if (rebuildSubstanceImmediately && (animateObjectBasedOnPosition == AnimateObjectBasedOnPosition.None || (animateObjectBasedOnPosition != AnimateObjectBasedOnPosition.None && playerDistanceToObject <= 10)))
                substance.RebuildTexturesImmediately();
        }
    }
    ...
    /*void Update()
    {
        if (animationToggle)
        {
            ...
            if (Input.GetKeyUp(KeyCode.Delete))
                DestroyImmediate(this.gameObject); //for testing
        }
    }
            ...
            if (materialVariables != null)
            {
                for (int i = 0; i < animatedMaterialVariables.Count; i++)
                {
                    ...
                    switch (propType)
                    {
                        case ProceduralPropertyType.Float:
                            {
                                // if (MaterialVariableKeyframeDictionaryList[currentKeyframeIndex].PropertyFloatDictionary[materialVariable.name] != MaterialVariableKeyframeDictionaryList[currentKeyframeIndex+1].PropertyFloatDictionary[materialVariable.name])
                                substance.SetProceduralFloat(materialVariable.name, Mathf.Lerp(MaterialVariableKeyframeDictionaryList[currentKeyframeIndex].PropertyFloatDictionary[materialVariable.name], MaterialVariableKeyframeDictionaryList[currentKeyframeIndex + 1].PropertyFloatDictionary[materialVariable.name], lerp * flickerFloatCalc));
                            }
                            break;
                        case ProceduralPropertyType.Color3:
                        ...
                        case ProceduralPropertyType.Enum:
                            {
                                if ((currentAnimationTime == 0 || currentAnimationTime == currentKeyframeAnimationTime))
                                {
                                    substance.SetProceduralEnum(materialVariable.name, MaterialVariableKeyframeDictionaryList[currentKeyframeIndex].PropertyEnumDictionary[materialVariable.name]);
                                }
                            }
