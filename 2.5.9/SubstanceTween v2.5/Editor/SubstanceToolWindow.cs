...
public class SubstanceToolWindow : EditorWindow
{
    ...
    static void Init()
    {
        ...
        //EditorWindow window = EditorWindow.GetWindow<T>(new Type[] { inspectorType });
        EditorWindow window = EditorWindow.GetWindow<SubstanceToolWindow>(new Type[] { inspectorType });
        //SubstanceToolWindow window = (SubstanceToolWindow)GetWindow(typeof(SubstanceToolWindow));
        Debug.Log(window);
...
//        currentSelection = obj;
        ...
        //Debug.Log(Selection.objects.Count());
        //Debug.Log(Selection.activeObject);
        //Debug.Log(Selection.activeGameObject == null);
        if (Selection.activeGameObject != null)
        {
            //Debug.Log(Selection.activeGameObject.GetComponent<Renderer>() != null);
            //Debug.Log(Selection.activeGameObject.activeSelf);
            //Debug.Log(Selection.activeGameObject && Selection.activeGameObject.GetComponent<Renderer>() != null && Selection.activeGameObject.activeSelf);
            if ( Selection.activeGameObject && Selection.activeGameObject.GetComponent<Renderer>() != null && Selection.activeGameObject.activeSelf)
            {
                currentSelection = Selection.activeGameObject;
                Debug.Log("Passed currentSelection: " + currentSelection);
                if (currentSelection)
                    rend = currentSelection.GetComponent<Renderer>();

                Debug.Log(currentSelection.GetComponent<PrefabProperties>() != null);
                if (currentSelection.GetComponent<PrefabProperties>() != null && currentSelection.GetComponent<PrefabProperties>().isActiveAndEnabled) // if object has the predab script get all of the keyframe/animation information from that script
                {
                    ...
                    if (rend)
                    {
                        ...
                        substanceCurve.keys = prefabScript.prefabAnimationCurve.keys;
    ...
    void OnSelectionChange() // Gets called whenever you change objects 
    {
        if (currentSelection != Selection.activeGameObject)
            substanceLerp = false;
        currentSelection = Selection.activeGameObject;
        if (currentSelection && currentSelection.GetComponent<PrefabProperties>() != null && Selection.activeGameObject.GetComponent<Renderer>() != null) // if a object is selected and it has a animated prefab script
        {
            prefabScript = currentSelection.GetComponent<PrefabProperties>();
            prefabScript.useSharedMaterial = true;
            prefabScript.animateBasedOnTime = false;
        ...
        if (EditorApplication.isPlaying && currentSelection && Selection.activeGameObject.GetComponent<Renderer>() != null) // If in playmode, object is selected and it has a renderer attached.
        {
            Debug.Log("AAA:" + currentSelection);
            if (currentSelection)
                rend = currentSelection.GetComponent<Renderer>();
            ...
            if (selectedStartupMaterials.Count > 0 && selectedStartupGameObjects.Count >0) // if at least one procedural material has been selected before 
            {
                ...
                {
                    Debug.Log("I: " + i + selectedStartupMaterials[i]);
                    if (currentSelection && currentSelection.name == selectedStartupGameObjects[i].name) // if currently selected object name = one of the gameobjects that you have already selected before
                    {
                        substance = selectedStartupMaterials[i];
                        materialExists = true; // object has already been selected before 
                    }
                    else if (!currentSelection)
                        Debug.Log("No current selection on object change");
    ...
    public void OnGUI() // where content gets displayed.
    {
        ...
        if (Event.current.type != EventType.ExecuteCommand || Event.current.commandName == "PopupMenuChanged" || Event.current.commandName == "ColorPickerChanged" || Event.current.commandName == "DelayedControlShouldCommit") //Drawing GUI while adding keys in the curve editor will give an error
        {
            ...
            if (EditorApplication.isPlaying && !gameIsPaused && currentSelection != null && rend && substance && AssetDatabase.GetAssetPath(Selection.activeObject) == String.Empty)//Check if you are in play mode, game is not paused, a object is selected and active , it has a renderer and the object is not in the project view
            {
                ...
                if (substance)//if object has a procedural material, loop through properties and create sliders/fields based on type
                {
                    ...
                    if (current.type == EventType.ContextClick)
                    {
                        ...
                        menu.AddItem(new GUIContent("About"), false, DisplayAboutDialog);
                        menu.ShowAsContext();
                        current.Use();
                    }
                    materialVariables = substance.GetProceduralPropertyDescriptions();
                    ...
                    if (MaterialVariableKeyframeList.Count >= 2)
                    {
                        ...
                        desiredAnimationTime = GUILayout.HorizontalSlider(animationTimeRestartEnd, 0, keyframeSum);
            ...
            else if (EditorApplication.isPlaying && selectedStartupMaterials.Count < 1) // when opening the tool for the first time and not having something thats required.
            {
                if (!rend && !Selection.activeObject)
                {
                    EditorUtility.DisplayDialog("Error", " No object selected.", "OK");
                    this.Close();
                    return;
                }
                else if (!Selection.activeGameObject.activeSelf)
                {
                    EditorUtility.DisplayDialog("Error", "Selected object is not visible/enabled", "OK");
                    this.Close();
                    return;
                }
                else if (!rend)
                ...
                {
                    this.Close();
                    EditorUtility.DisplayDialog("Error", " No ProceduralMaterial/Substance attached to object.", "OK");
                    return;
                }
            ...
            else if (EditorApplication.isPlaying && AssetDatabase.GetAssetPath(Selection.activeObject) != String.Empty && PrefabUtility.GetPrefabType(Selection.activeObject) != PrefabType.None)
                EditorGUILayout.LabelField("Prefab is currently selected in the Project view, Move Prefab to a scene and select it in the Hierarchy window", errorTextStyle);
            else if (EditorApplication.isPlaying && Selection.activeObject && PrefabUtility.GetPrefabType(Selection.activeObject) == PrefabType.None)
                EditorGUILayout.LabelField("Wrong type of object selected", errorTextStyle);
            else if (EditorApplication.isPlaying && Selection.activeObject && currentSelection.GetComponent<Renderer>() == null)
                EditorGUILayout.LabelField("This object has no renderer attached.", errorTextStyle);
            else if (!Selection.activeObject)
                EditorGUILayout.LabelField("No object selected", errorTextStyle);
    ...
    void OnDisable()
    {
        if (!EditorApplication.isPlaying)
        {
            Debug.Log("Destroy when not in play mode.");
            MaterialVariableKeyframeDictionaryList.Clear();
            MaterialVariableKeyframeList.Clear();
            keyFrames = 0;
           
            this.Close(); // error when starting unity
            UpdatingStartVariables = true;
        }
    }
    ...
    { // Show keyframe controls(animation curve, select/remove/overwrite keyframes) in the GUI
        ...
        //Debug.Log(keyframeTimeList.Count() + " : " + substanceCurve.keys.Count());
    ...
    {
        if (cacheSubstance && materialVariables != null && substance)
        {
        ...
        else if (!cacheSubstance && materialVariables != null && substance) // add variables if the animate but do not cache them.
        {
    ...
    void SavePrefab() // Saves a Prefab to a specified folder
    {
        ...
        if (prefabPath.Length != 0)
        {
            ...
            prefabRend.sharedMaterial.enableInstancing = true;// enabled GPU instancing 
#endif 
