...
public class SubstanceTool : Editor
{
    ...
    public void OnEnable()
    {
        ...
        toolbarPopupStyle = new GUIStyle(EditorStyles.toolbarPopup);
           styleAnimationButton = new GUIStyle(EditorStyles.toolbarButton);
            ...
            if (!Selection.activeGameObject.GetComponent<PrefabProperties>())// should only be called after the first time you open the tool
            {
    ...
    public override void OnInspectorGUI() // where content gets displayed.
    {
        ...
        if (Application.isPlaying && Selection.activeGameObject && Selection.activeGameObject.GetComponent<Renderer>() && Selection.activeGameObject.GetComponent<Renderer>().sharedMaterial)
        {
            ...
            if (Event.current.type != EventType.ExecuteCommand || Event.current.commandName == "PopupMenuChanged" || Event.current.commandName == "ColorPickerChanged" || Event.current.commandName == "DelayedControlShouldCommit") //Drawing GUI while adding keys in the curve editor will give an error
            {
                ...
                /* else if (AssetDatabase.GetAssetPath(Selection.activeObject) != String.Empty && PrefabUtility.GetPrefabType(Selection.activeObject) != PrefabType.None)
                     EditorGUILayout.LabelField("SubstanceTween: Prefab is currently selected in the Project view, Move Prefab to a scene and select it in the Hierarchy window", errorTextStyle);
                 else if (Selection.activeObject && PrefabUtility.GetPrefabType(Selection.activeObject) == PrefabType.None)
                     EditorGUILayout.LabelField("SubstanceTween: Wrong type of object selected", errorTextStyle);
                 else if (Selection.activeObject && currentSelection.GetComponent<Renderer>() == null)
                     EditorGUILayout.LabelField("SubstanceTween: This object has no renderer attached.", errorTextStyle);
                 else if (!Selection.activeObject)
                     EditorGUILayout.LabelField("SubstanceTween: No object selected", errorTextStyle);
                 if (selectedStartupMaterials.Count >= 1 && !substance)
                     EditorGUILayout.LabelField("SubstanceTween: No ProceduralMaterial attached to object, Select another object", errorTextStyle); */
                if (EditorGUI.EndChangeCheck() || substanceLerp)
                {
                    Repaint();
                }
        ...
        if (EditorApplication.isPlaying)
        {
            if (substance && (substanceLerp) && currentSelection == Selection.activeGameObject)
            {
                ...
                if (animationType == AnimationType.Loop)// animate through every keyframe and repeat at the beginning
                {
                    ...
                    {
                        Debug.Log(substanceCurve.keys[currentKeyframeIndex].time + " " + substanceCurve.keys[currentKeyframeIndex + 1].time);
                        lerp = Mathf.InverseLerp(substanceCurve.keys[currentKeyframeIndex].time, substanceCurve.keys[currentKeyframeIndex + 1].time, curveFloat);
                    }
                }
    ...
    void DisplayControlForParameter(ProceduralPropertyDescription parameter) // sorts parameters by type and displays controls for that type in the GUI
    {
        ...
        {
            //if (substanceCurve.keys.Length <= 1 || !substanceLerp || (substanceLerp && animatedMaterialVariables != null && animatedMaterialVariables.Contains(parameter)) )
            {
                EditorGUI.BeginChangeCheck();
                GUILayout.BeginHorizontal(GUILayout.Width(EditorGUIUtility.currentViewWidth * 0.90f));
                GUILayout.Label(parameter.name + '(' + parameter.label + ')');
